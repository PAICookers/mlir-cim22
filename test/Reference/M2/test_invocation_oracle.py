...
def mapping_text(mapping):
    ...
    return (f"{{core_coord = {array(core)}, destination = {array(destination)}, "
            f"ingress = {array(ingress)}, route = {array(route)}, "
            f"source = {array(source)}}}")


def profile_attrs():
    return (f'cim.target_profile = "{oracle.PROFILE_ID}", '
            f"cim.target_profile_version = {oracle.PROFILE_VERSION} : i64")


def work_attrs(work):
    return (f"cim.mapping = {mapping_text(work.mapping)}, core_slot = {work.core_slot} : i64, "
...
def render_dump(groups=((WORK0, WORK1),)):
    ...
    for group in groups:
        for work in group:
            ...
            lines.append(f'    "cim.configure_input"(%input) {{{attrs}}} '
                         ": (tensor<64xi8>) -> ()")
            lines.append(f'    "cim.configure_weight"() {{{attrs}, weight = @w0}} '
                         ": () -> ()")
        for work in group:
            lines.append(f'    "cim.dispatch"() {{{work_attrs(work)}}} : () -> ()')
        first = group[0]
        once_attrs = (f"cim.mapping = {mapping_text(first.mapping)}, "
                      f"core_slot = {first.core_slot} : i64, "
                      f"group_id = {first.group_id} : i64")
        lines.append(f'    "cim.once"() {{{once_attrs}}} : () -> ()')
        for work in group:
            result = f"%read{work.work_id}"
            lines.append(f'    {result} = "cim.readback"() {{{work_attrs(work)}}} '
                         ": () -> tensor<16xi21>")
            readbacks.append(result)
        barrier_attrs = f"group_id = {first.group_id} : i64"
        lines.append(f'    "cim.group_barrier"() {{{barrier_attrs}}} : () -> ()')
    lines.append(f"    %wide0 = arith.extsi {readbacks[0]} : tensor<16xi21> to tensor<16xi32>")
    if len(readbacks) > 1:
        lines.append(f"    %wide1 = arith.extsi {readbacks[1]} : tensor<16xi21> to tensor<16xi32>")
...
class InvocationOracleTest(unittest.TestCase):
    ...
    def test_function_and_static_resource_faults(self):
        ...
        for faulty, error in faults:
            with self.subTest(error=error), self.assertRaisesRegex(oracle.OracleError, error):
                oracle.validate_dump(faulty)

    def test_configuration_faults(self):
        text = render_dump()
        missing_work = mutate_line(
            text, "cim.configure_input", "work_id = 0 : i64", "missing_work = 0 : i64")
        invalid_macro = mutate_line(
            text, "cim.configure_input", "macro_slot = 0 : i64", "macro_slot = 2 : i64")
        nonzero_copy = mutate_line(
            text, "cim.configure_input", "route = array<i64: 0, 0, 0, 0, 0, 0>",
            "route = array<i64: 0, 0, 0, 1, 0, 0>")
        same_macro = text.replace("macro_slot = 1 : i64", "macro_slot = 0 : i64")
        missing_config = "\n".join(
    ...
    def test_once_readback_barrier_and_group_faults(self):
        text = render_dump()
        once_macro = text.replace('"cim.once"() {',
                                  '"cim.once"() {macro_slot = 0 : i64, ', 1)
        once_line = next(line for line in text.splitlines() if "cim.once" in line)
        duplicate_once = text.replace(once_line, once_line + "\n" + once_line, 1)
        wrong_readback = text.replace("-> tensor<16xi21>", "-> tensor<16xi32>", 1)
        barrier_lines = text.splitlines()
        barrier_index = next(index for index, line in enumerate(barrier_lines)
                             if "cim.group_barrier" in line)
        read_index = next(index for index, line in enumerate(barrier_lines)
                          if "cim.readback" in line)
        barrier_lines[barrier_index], barrier_lines[read_index] = (
            barrier_lines[read_index], barrier_lines[barrier_index])
        barrier_early = "\n".join(barrier_lines)
        two_groups = render_dump(((WORK0,), (WORK2,)))
        group0_barrier = next(line for line in two_groups.splitlines()
