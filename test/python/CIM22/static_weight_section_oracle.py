...
WORK_FIELDS = (
    "m_tile", "n_tile", "k_tile", "work_id", "group_id", "core_slot",
    "macro_slot",
)
...
def _array(body: str, name: str, bits: int, size: int,
           line_number: int) -> tuple[int, ...]:
    match = re.search(
...
def _mapping(body: str, name: str, line_number: int) -> tuple[tuple[int, ...], ...]:
    ...
    mapping = tuple(
        ...
        for field, size in (
            ("core_coord", 2), ("ingress", 2), ("source", 2),
            ("destination", 2), ("route", 6),
        )
    )
...
def _parse_dense(line: str, line_number: int) -> tuple[str, np.ndarray] | None:
    match = re.search(
        ...
        r"dense<(.*)>\s*:\s*tensor<16x64xi8>", line)
    if not match:
        return None
    payload = match.group(2).strip()
    raw = re.fullmatch(r'"0x([0-9A-Fa-f]+)"', payload)
    if raw:
        values = np.frombuffer(bytes.fromhex(raw.group(1)), dtype=np.uint8).copy()
    else:
...
def _parse_configure(line: str, function: str,
                     line_number: int) -> Record | None:
    match = re.search(
    ...
    return Record(function, match.group(1), work, mapping, mapping[4], work[-1], None)
...
def _parse_command(line: str, line_number: int) -> Record | None:
    ...
    if names != PROVENANCE_FIELDS:
        raise OracleError(f"line {line_number}: provenance fields mismatch: {sorted(names)}")
    function = _reference(provenance, "function", line_number)
    resource = _reference(provenance, "resource", line_number)
    work = tuple(_integer(provenance, name, 64, line_number) for name in WORK_FIELDS)
    ...
    if words.size != 256 or np.any(words < -(1 << 31)) or np.any(words >= (1 << 31)):
        raise OracleError(f"line {line_number}: words expects 256 signed i32 values")
    return Record(function, resource, work, mapping, route, macro,
                  words.astype(np.int32))


def validate_lines(lines, expected_commands: int | None = None) -> tuple[int, int]:
    weights = {}
    configurations = []
    commands = []
    function = ""
    for line_number, line in enumerate(lines, 1):
        if line.lstrip().startswith("//"):
            continue
        if re.search(r"\b(?:uint64|flit|flatbuffer|runtime)\b", line, re.IGNORECASE):
            raise OracleError("typed M2.4b dump contains a forbidden raw/runtime marker")
        ...
        if dense:
            if dense[0] in weights:
                raise OracleError(f"line {line_number}: duplicate resource {dense[0]}")
    ...
    for index, (command, configure) in enumerate(zip(commands, configurations)):
        if command[:-1] != configure[:-1]:
            raise OracleError(f"command {index}: provenance/route/Macro mismatch")
        if command.resource not in weights:
            raise OracleError(f"command {index}: unknown resource {command.resource}")

    for start in range(0, len(commands), 128):
        chunk = commands[start:start + 128]
        expected = map_tiles(np.stack([weights[command.resource] for command in chunk]))
...
def validate_dump(text: str, expected_commands: int | None = None) -> tuple[int, int]:
    return validate_lines(text.splitlines(), expected_commands)


def main() -> None:
    parser = argparse.ArgumentParser(description="Validate an M2.4b typed section")
