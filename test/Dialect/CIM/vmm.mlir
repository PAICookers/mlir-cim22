...
func.func @identified_vmm(%input: tensor<64xi8>, %weight: tensor<16x64xi8>) {
  %0 = cim.vmm %input, %weight {cim.transaction_idx = 0 : i64, m_tile = 0 : i64, n_tile = 1 : i64, k_tile = 2 : i64} : tensor<64xi8>, tensor<16x64xi8> -> tensor<16xi21>
  return
}

// CHECK-LABEL: func.func @scheduled_vmm(
// CHECK: cim.vmm {{.*}} {cim.transaction_idx = 0 : i64, group_id = 3 : i64, k_tile = 1 : i64, m_tile = 0 : i64, n_tile = 1 : i64, work_id = 7 : i64}
func.func @scheduled_vmm(%input: tensor<64xi8>, %weight: tensor<16x64xi8>) {
  %0 = cim.vmm %input, %weight {cim.transaction_idx = 0 : i64, m_tile = 0 : i64, n_tile = 1 : i64, k_tile = 1 : i64, work_id = 7 : i64, group_id = 3 : i64} : tensor<64xi8>, tensor<16x64xi8> -> tensor<16xi21>
  return
}
